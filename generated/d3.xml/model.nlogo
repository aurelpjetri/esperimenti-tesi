...
movers-own [
  ...
  movers-state ;; used for data gathering
]
...
beacons-own [
  ...
  entry-rate
  exit-rate
  ;; needed for orienting new mover to the closest beacon
  influence-area

  ;; needed for results registration
  beacon-side
]
...
to setup-uni
  default-setup
  generate-uniform-movers global-movers
end
;; GO PROCEDURE
;; ============

to go
  show (word "run: " behaviorspace-run-number " - round: " ticks " - movers: " count movers)
    ...
    ask patches in-radius intersection-radius [
      ask movers-here [
        if random-float 1 < temp-exit-rate and empty? destination-list [
          ...
          if regenerative-model? = true [generate-uniform-movers 1]
          die
        ]]
    ]
  ]
  ...
  ask beacons with [entry-point? = true and (entry-infinity? = true or entry-limit > 0)] [
    ...
    ask patches in-radius intersection-radius with [count movers-here < global-crowd-max-at-patch] [
      if random-float 1 < temp-entry-rate [
        generate-new-mover entry-beacon
...
to generate-new-mover [entry-beacon]
  sprout-movers 1 [
    set current-beacon entry-beacon
    set mover-behavior get-random-mover-behaviour [entry-percentages] of current-beacon
    standard-mover-settings
  ]
end

to generate-uniform-movers [n]
  ask one-of patches with [wall = false and count movers-here < global-crowd-max-at-patch] [
    sprout-movers 1 [
      set current-beacon [claimed?] of patch-here
      set mover-behavior 0
      standard-mover-settings
    ]
  ]
  if n > 1 [generate-uniform-movers n - 1]
end
...
to standard-mover-settings
  ...
  ifelse not empty? destination-list
    ...
    [ ;; we have to reset the context that might have been poluted by others
      nw:set-context (beacons) (link-set streets directed-streets)
      set-destination-to-nearest-exit]
...
;; BETTER BEACON FINDING FOR MOVERS
;; ================================

to set-destination-to-nearest-exit
  set destination-beacon min-one-of (beacons with [exit-point? = true]) [nw:weighted-distance-to [current-beacon] of myself "weight"]
end

to calculate-influence-area
  ask beacons [set influence-area patch-here]
  ask patches [set claimed? false]
  iter-influence-area
end
to iter-influence-area
  let keep false
  while [keep = false] [
    let change-row []
    ask beacons [
      let bp border-patches
      ifelse any? bp
        [ ask bp [set claimed? myself]
          set influence-area (patch-set influence-area bp)
          set change-row lput false change-row]
        [ set change-row lput true change-row]     ]
    set keep (reduce [?1 and ?2] change-row)
  ]
end
to-report border-patches
  let b-patches no-patches
  ask influence-area [
    set b-patches (patch-set b-patches (neighbors with [wall = false and claimed? = false]))
  ]
  report b-patches
end
...
to orient-random-mover [random-mover]
  ;; change the destination beacon if i'm trying to get out, i.e. no more destinations
  if empty? destination-list [
    nw:set-context (beacons) (link-set streets directed-streets)
    ifelse count beacons with [exit-point? = true] > 1
      [set destination-beacon min-one-of (beacons with [exit-point? = true and self != [destination-beacon] of myself]) [nw:weighted-distance-to [current-beacon] of myself "weight"]]
      [set-destination-to-nearest-exit]
    set color [color] of destination-beacon
  ]
...
to mark-beacons
  let llmostt min-one-of beacons [xcor + ycor]
  let rrmostt max-one-of beacons [xcor + ycor]
  let x1 [xcor] of llmostt
  let x2 [xcor] of rrmostt
  let y1 [ycor] of llmostt
  let y2 [ycor] of rrmostt
  let a1 calca x1 x2 y1 y2
  let b1 calcb x1 y1 a1

  let a2 (- a1)
  let b2 calcb x1 y2 a2

  ask beacons [
    ifelse ycor < (a1 * xcor + b1)
      [ ifelse ycor < (a2 * xcor + b2)
          [set beacon-side "south"]
          [set beacon-side "east"]]
      [ ifelse ycor < (a2 * xcor + b2)
          [set beacon-side "west"]
          [set beacon-side "north"]]
  ]
end

to-report calca [xmin xmax ymin ymax]
  report (ymax - ymin) / (xmax - xmin)
end
to-report calcb [x y a]
  report (y - (x * a))
end
...
to register-results
  ;; delete the file from the previous runs
  let filename (word results-dir "/" "experiment_" behaviorspace-experiment-name "_run_" behaviorspace-run-number ".csv")
  if file-exists? filename [file-delete filename]
  ;; open a file
  file-open filename

  let onei first table:keys global-movers-results
  file-print to-csv (list "id" (to-csv map [item 0 ?] table:get global-movers-results onei))
...
to movers-data-setup
  ...
  ;foreach sort beacons with [exit-point? = true] [ table:put movers-data (word "exit_" ?) -1 ]
  foreach ["south" "east" "north" "west"] [table:put movers-data (word "exit_" ?) -1 ]
end
...
to movers-data-collect [passing-beacon]
  ...
  if table:get movers-data (word "exit_" [beacon-side] of passing-beacon) = -1
     [table:put movers-data (word "exit_" [beacon-side] of passing-beacon) ticks]
end
...
to default-configuration
  ...
  table:put behaviors-map 0 get-interest-beacons map [ list (world-offset + item 0 ?) (world-offset + item 1 ?) ] []
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="import-filename">
      <value value="&quot;netlogo.map&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="results-dir">
      <value value="&quot;results&quot;"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="regenerative-model?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="global-movers">
      <value value="51"/>
      <value value="501"/>
      <value value="5001"/>
    </enumeratedValueSet>
